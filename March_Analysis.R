...
dat <- fread("data/working_copies/MarchMUNISENSE_CASPER_DAVIS_COMBINED_2023_03copy.csv")
...
#filter out wind speeds of more than 12 m/s
...
write.csv(dat, "data/working_copies/filtered/March_Filtered.csv", row.names=FALSE)


#Split microphones by arrival or departure 
...
# DATA ANALYSIS 
#adding extra columns for relative difference analysis 
...
mar_arr <- data.frame(dat_arr$date_sound, dat_arr$rel_diff_5, 
...
mar_dep <- data.frame(dat_dep$date_sound, dat_dep$rel_diff_5, 
...
write.csv(mar_arr, "data/working_copies/split_datasets/Mar_Arrivals.csv", row.names=FALSE)
write.csv(mar_dep, "data/working_copies/split_datasets/Mar_Departures.csv", row.names=FALSE)
